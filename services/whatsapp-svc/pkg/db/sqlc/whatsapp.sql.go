...
const getBusinessPhone = `-- name: GetBusinessPhone :one
SELECT
  id,
  company_id,
  phone,
...
func (q *Queries) GetBusinessPhone(ctx context.Context, lower string) (WhatsappBusinessPhone, error) {
	row := q.db.QueryRowContext(ctx, getBusinessPhone, lower)
	var i WhatsappBusinessPhone
	err := row.Scan(
		&i.ID,
		&i.CompanyID,
